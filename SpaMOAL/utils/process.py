...
def read_label(input_folder,dataset):
    data = pd.read_csv(input_folder+dataset+"_groundtruth.csv")
    # 获取不同的类别
    unique_classes = data['Ground_Truth'].unique()
    # 打印类别数量和类别列表
    num_classes = len(unique_classes)
    print(f"共有 {num_classes} 种类别")
    # for i, class_name in enumerate(unique_classes):
    #     print(f"类别 {class_name}: 编码 {i}")
    # 使用get_dummies将类别转换为one-hot编码
    one_hot_matrix = pd.get_dummies(data['Ground_Truth'])
...
def load_10x(input_folder, omics_list,omics_dim, sc=3, dataset=None, model="none", omics=None, distance=None, adj_neighbor=6):
    ...
    label = read_label(input_folder, dataset)
    label = label.values

...
parser.add_argument('--destination', default='../Data/common_adj.csv')
...
# 创建一个新的邻接矩阵来存储并集的边
union_adj = np.zeros_like(adj1)

# 遍历两个邻接矩阵，找出并集的边
for i in range(adj1_row):
    for j in range(adj1_column):
        if adj1[i][j]!= 0 or adj2[i][j]!= 0:
            union_adj[i][j] = 1 if adj1[i][j]!= 0 or adj2[i][j]!= 0 else 0

# 将 union_adj 保存为 CSV 文件
pd.DataFrame(union_adj).to_csv(args.destination, header=None, index=None)
print(f"Saved union adjacency matrix to {args.destination}")
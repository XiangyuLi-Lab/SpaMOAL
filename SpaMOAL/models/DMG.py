...
class DMG(embedder):
    ...
    def training(self):
        ...
        predictor =Predictor(self.args.c_dim+self.args.p_dim*self.args.num_view,self.args.projector_dim,self.args.n_clusters).to(device)
        ...
        a.to_csv(f"/home/lixiangyu/DMG/DMG-main/benchmark/{self.args.dataset}/{dataset_folder}/{self.args.model}_{self.args.dataset}_{self.args.alpha}_{self.args.beta}_{self.args.lammbda}_{self.args.num_iters}_mclust_{self.args.n_clusters}_DMG_embedding.csv")
        return a
...
def trainmultiplex(model,predictor, mea_func, U, features, adj_list,idx_p_list, args,  optimizer, device, epoch):
    ...
    loss_con = loss_contrastive(U, private, adj_list,predictions , args)
    ...
    #print(match_err,recons_err,recons_nei,corr,loss_con,self_entropy,DDC)
    loss = match_err +  args.alpha*(recons_err+recons_nei) - args.beta* corr + args.lammbda * loss_con + self_entropy + DDC

    #loss =   args.alpha*(recons_err+recons_nei) - args.beta* corr + args.lammbda * loss_con + self_entropy + DDC
    #loss = match_err  - args.beta* corr + args.lammbda * loss_con + self_entropy + DDC
    #loss = match_err +  args.alpha*(recons_err+recons_nei)  + args.lammbda * loss_con + self_entropy + DDC
    #loss = match_err +  args.alpha*(recons_err+recons_nei) - args.beta* corr  + self_entropy + DDC

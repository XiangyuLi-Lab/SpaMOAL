...
class DMG(embedder):
    ...
    def training(self, rank, world_size):
        seed = self.args.seed
        np.random.seed(seed)
        ...
        predictor = Predictor(self.args.c_dim + self.args.p_dim * 2, self.args.projector_dim,
                              self.args.n_clusters).to(device)
        ...
        # 使用 DistributedDataParallel 并行化模型
        ae_model = torch.nn.parallel.DistributedDataParallel(ae_model, device_ids=[rank])
        predictor = torch.nn.parallel.DistributedDataParallel(predictor, device_ids=[rank])
        mea_func = [torch.nn.parallel.DistributedDataParallel(model, device_ids=[rank]) for model in mea_func]
        ...
        for itr in tqdm(range(1, self.args.num_iters + 1)):
            ...
            for innerepoch in range(self.args.inner_epochs):
                # Backprop to update
                loss, match_err, recons, corr, contrastive, self_entropy, DDC, common, private = trainmultiplex(
                    ae_model, predictor, mea_func, U, features, adj_list, self.idx_p_list, self.args, optimizer, device,
                    itr * innerepoch)
            if loss < best:
                best = loss
                cnt_wait = 0
            ...
            print(f"Rank {rank}: ====> Iteration: {itr} Loss = {loss:.4f}")
        if self.args.use_pretrain:
            if isinstance(ae_model, torch.nn.parallel.DistributedDataParallel):
                ae_model.module.load_state_dict(
        ...
        if rank == 0:
            ...
            a.to_csv(
                f"/home/lixiangyu/DMG/DMG-main/benchmark/{self.args.dataset}/{dataset_folder}/{self.args.model}_{self.args.dataset}_{self.args.alpha}_{self.args.beta}_{self.args.lammbda}_{self.args.num_iters}_mclust_DMG_embedding.csv")
